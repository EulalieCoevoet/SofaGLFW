...
void ImGuiGUIEngine::initBackend(GLFWwindow* glfwWindow)
{
    ...
    if (monitor)
    {
        ...
        ImFontConfig configOSR;
        configOSR.GlyphOffset.y = -1.f;
        ImFontConfig configFA;
        configFA.MergeMode = true;
        configFA.GlyphOffset.y = -2.f;
        configFA.GlyphMinAdvanceX = .0f; // Use if you want to make the icon monospaced
        ImFontConfig configDejaVu(configFA);
        configDejaVu.GlyphOffset.y = -4.f;
        static const ImWchar icon_rangesFA[] = { 0x0001, ICON_MIN_FA-1, ICON_MAX_FA+1, 0xfffe, 0 };
        static const ImWchar icon_rangesDejaVu[] = { 0x0001, 0x2021, 0x2b55, 0xfffe, 0};

        configFA.GlyphExcludeRanges = icon_rangesFA;
        configDejaVu.GlyphExcludeRanges = icon_rangesDejaVu;
        io.Fonts->AddFontFromMemoryCompressedTTF(OpenSans_Regular_compressed_data, OpenSans_Regular_compressed_size, 18 * yscale, &configOSR);
